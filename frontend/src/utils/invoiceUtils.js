...
export const downloadInvoicePdf = async (booking, role = 'customer') => {
  const bookingId = booking.bookingId || String(booking._id || '').slice(-6) || 'booking';
  const invoiceId = booking.invoice?.invoiceId || `INV-${bookingId}`;
  ...
  try {
    ...
    doc.setFontSize(24);
    doc.text('DriveEase Invoice', 40, y);

    y += 22;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(`Invoice: ${invoiceId}`, 40, y);
    doc.text(`Booking: ${bookingId}`, 260, y);
    doc.text(`Date: ${dateStr}`, 430, y);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(11);
    doc.text(`${customerName} (${customerPhone})`, 40, y);
    doc.text(driverName, 300, y);
    ...
    y += 14;
    doc.setFont('helvetica', 'normal');
    doc.text(`${routeStart}  ->  ${routeEnd}`, 40, y, { maxWidth: 520 });

    y += 30;
    doc.setFont('helvetica', 'bold');
    doc.text('Fare Summary', 40, y);
    y += 16;
    doc.setFont('helvetica', 'normal');
    doc.text(`Subtotal`, 40, y);
    doc.text(`INR ${subtotal}`, 520, y, { align: 'right' });
    y += 16;
    doc.text(`Insurance Add-on`, 40, y);
    doc.text(`INR ${insurance}`, 520, y, { align: 'right' });
    y += 16;
    doc.text(`Payment Method`, 40, y);
    doc.text(paymentMethod, 520, y, { align: 'right' });

    y += 22;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.text('Total Paid', 40, y);
    doc.text(`INR ${total}`, 520, y, { align: 'right' });
    ...
    doc.text(
      role === 'driver'
        ? 'Driver earnings statement generated by DriveEase.'
        : 'Customer trip invoice generated by DriveEase.',
      40,
      y
    );

    doc.save(`DriveEase-Invoice-${invoiceId}.pdf`);
  } catch (_) {
    // Fallback for environments where jsPDF is unavailable.
    downloadInvoice(booking, role);
  }
};
